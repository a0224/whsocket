...
import com.hao.socket.client.impl.client.action.ActionDispatcher;
import com.hao.socket.client.sdk.WhSocket;
...
import com.hao.socket.client.sdk.client.connection.NoneReconnect;
...
public class ComplexDemoActivity extends AppCompatActivity {
    ...
    private SocketActionAdapter adapter = new SocketActionAdapter() {
        ...
        private void initSwitch() {
            WhSocketOptions okSocketOptions = mManager.getOption();
    ...
    private void initData() {
        ...
        WhSocketOptions.Builder builder = new WhSocketOptions.Builder();
        builder.setReconnectionManager(new NoneReconnect());
        builder.setCallbackThreadModeToken(new WhSocketOptions.ThreadModeToken() {
            @Override
            public void handleCallbackEvent(ActionDispatcher.ActionRunnable runnable) {
                handler.post(runnable);
            }
        });
        mManager = WhSocket.open(mInfo).option(builder.build());
    }
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (!isChecked) {
                    if (!(mManager.getReconnectionManager() instanceof NoneReconnect)) {
                        mManager.option(new WhSocketOptions.Builder(mManager.getOption()).setReconnectionManager(new NoneReconnect()).build());
                        logSend("关闭重连管理器(Turn Off The Reconnection Manager)");
                    }
                } else {
                    if (mManager.getReconnectionManager() instanceof NoneReconnect) {
                        mManager.option(new WhSocketOptions.Builder(mManager.getOption()).setReconnectionManager(WhSocketOptions.getDefault().getReconnectionManager()).build());
        ...
        mSetFrequency.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    WhSocketOptions okOptions = new WhSocketOptions.Builder(mManager.getOption())
